/**
 * MediLink UI primitives - platform admin.
 *
 * A third copy, after web-patient and web-provider. The note on the provider
 * copy said to promote these to a shared package if a third surface appeared.
 * This is that surface - but the right move is still not a package: three Vite
 * apps with no monorepo tooling would each need a build step and a watch mode
 * to save an import path, and the file has not diverged.
 *
 * What WOULD justify it: the first time a fix has to be made in three places.
 * Until then this is copied code that nobody has had to maintain twice.
 */
...
/**
 * Placeholder for a table while it loads. Workspace-only: the patient app has
 * no tables, and a card skeleton standing in for a table row makes the layout
